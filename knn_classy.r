...
test.data <- data[-train.index, ]       # note use of neg index...different than Python!s
...
STEP 1:
   ...
   function(t){
   Random r = random(size,interval)   #random generator ---like this seq(0,150,10)
   list_of_random_numbers = rList
   for item in rList {
   item ---training set
   t -item ----test set
   =========
   form a list with values of item and t-item as pairs (training/test)
   ...
   perform knn classification on n times (n being the length of list in step 1)
   store values in another list
   
   STEP3:
   # n-fold generalization error = average over all iterations
   calculate generalization error from values obtained in STEP 3
}
...
Now add ticks every 10 data points:
results.plot <- ggplot(results, aes(x=k, y=err.rate)) + geom_point() + geom_line()  + scale_x_continuous(limits=c(0,100), breaks=seq(0,100,10) )
> print(results.plot)
